...
/* =========================================================
   TransiSafe 2.0 - Entwicklungsschritt 13
   ...
   - Management-KPIs in management_summary.csv
   - Attention Required, Priority-Faelle und Management-Ranking
   ========================================================= */
...
#define DEFAULT_MANAGEMENT_SUMMARY_FILE_PATH "management_summary.csv"
...
static void set_default_config(AppConfig* config)
{
    ...
    copy_text(
        config->management_summary_file_path,
        sizeof(config->management_summary_file_path),
        DEFAULT_MANAGEMENT_SUMMARY_FILE_PATH);
    ...
    size_t error_message_size)
{
    ...
    if (strcmp(key, "management_summary_file") == 0) {
        if (!copy_text(
            config->management_summary_file_path,
            sizeof(config->management_summary_file_path),
            value)) {
            snprintf(
                error_message,
                error_message_size,
                "Ungueltiger oder zu langer management_summary_file-Pfad.");
    ...
    StatisticsResult* statistics)
{
    ...
    statistics->highest_priority_value = 99;
    statistics->highest_priority_criticality_score = -DBL_MAX;
    statistics->highest_priority_case_index = 0;
    statistics->highest_priority_status = STATUS_SAFE;

    statistics->lowest_safety_margin_score = DBL_MAX;
    statistics->highest_safety_margin_score = -DBL_MAX;
...
static double calculate_safety_margin_score(
    const OperatingPoint* point,
    const AnalysisResult* result)
{
    double normalized_power_margin;
    double normalized_temperature_margin;

    normalized_power_margin =
        result->power_margin_w /
        point->model->p_max;
    ...
    return normalized_power_margin < normalized_temperature_margin
        ? normalized_power_margin
        : normalized_temperature_margin;
}
    ...
    const AnalysisResult* result)
{
    ...
    safety_margin_score =
        calculate_safety_margin_score(point, result);

    if (priority == 1) {
        statistics->priority_1_count++;
    }
    else if (priority == 2) {
        statistics->priority_2_count++;
    }
    else if (priority == 3) {
        statistics->priority_3_count++;
    }
    else if (priority == 4) {
        statistics->priority_4_count++;
    }

    if (priority < statistics->highest_priority_value ||
        (priority == statistics->highest_priority_value &&
            criticality_score >
            statistics->highest_priority_criticality_score)) {

        statistics->highest_priority_value = priority;
        statistics->highest_priority_criticality_score =
            criticality_score;
        statistics->highest_priority_case_index =
            case_index;
        statistics->highest_priority_status =
            result->status;

        copy_text(
            statistics->highest_priority_transistor_id,
            sizeof(statistics->highest_priority_transistor_id),
            point->model->transistor_id);
    }

    if (safety_margin_score <
        statistics->lowest_safety_margin_score) {
        statistics->lowest_safety_margin_score =
            safety_margin_score;

        copy_text(
            statistics->lowest_safety_margin_transistor_id,
            sizeof(statistics->lowest_safety_margin_transistor_id),
            point->model->transistor_id);
    }

    if (safety_margin_score >
        statistics->highest_safety_margin_score) {
        statistics->highest_safety_margin_score =
            safety_margin_score;

        copy_text(
            statistics->highest_safety_margin_transistor_id,
            sizeof(statistics->highest_safety_margin_transistor_id),
            point->model->transistor_id);
    }
...
static int write_management_summary_csv(
    const char* file_path,
    const StatisticsResult* statistics)
{
    FILE* file;
    int attention_required_count;
    double attention_required_percent;

    file = fopen(file_path, "w");

    if (file == NULL) {
        printf(
            "Fehler: Management-Datei konnte nicht geschrieben werden: %s\n",
            file_path);

        write_log(
            LOG_ERROR,
            "Management-Datei konnte nicht geschrieben werden: %s",
            file_path);
    ...
    fprintf(
        ...
        "attention_required_percent,"
        "highest_risk_model,"
        "lowest_safety_margin_model,"
        "highest_safety_margin_model,"
        "high_priority_cases,"
    ...
    if (statistics->total_count > 0) {
        fprintf(
            ...
            statistics->priority_3_count,
            statistics->highest_priority_case_index,
            recommended_action_from_status(
                statistics->highest_priority_status));
    }
    else {
        fprintf(
            file,
            "0.000000,NONE,NONE,NONE,0,0,0,0,No data\n");
    }
...
static void print_management_summary(
    const StatisticsResult* statistics)
{
    int attention_required_count;
    ...
    printf(
        "Attention Required:             %.2f %%\n",
        percentage(
            attention_required_count,
            statistics->total_count));

    printf(
        "Highest Risk Model:             %s\n",
        statistics->highest_priority_transistor_id);

    printf(
        "Lowest Safety Margin Model:     %s\n",
        statistics->lowest_safety_margin_transistor_id);

    printf(
        "Highest Safety Margin Model:    %s\n",
        statistics->highest_safety_margin_transistor_id);

    printf(
        "High Priority Cases (P1):       %d\n",
        statistics->priority_1_count);

    printf(
        "Medium Priority Cases (P2):     %d\n",
        statistics->priority_2_count);

    printf(
        "Low Priority Cases (P3):        %d\n",
        statistics->priority_3_count);

    printf(
        "Highest Priority Case:          %d\n",
        statistics->highest_priority_case_index);
...
static int run_csv(void)
{
    ...
    print_management_summary(&statistics);
    ...
    if (write_management_summary_csv(
        g_config.management_summary_file_path,
        &statistics)) {
        printf(
            "Management-Datei: %s\n",
            g_config.management_summary_file_path);
    ...
    write_log(
        ...
        "SummaryFile=%s | ManagementSummaryFile=%s",
        statistics.total_count,
        statistics.safe_count,
        statistics.critical_count,
        total_not_safe_count(&statistics),
