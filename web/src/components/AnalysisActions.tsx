...
import type { AnalysisInput, AnalysisResponse, Mode, RecentAnalysis } from "../types";
import { WhatIfModal } from "./WhatIfModal";

export function AnalysisActions({ input, mode, onModeChange, disabled, loading, result, recentAnalyses, onOpenRecent, onRunWhatIf, onApplyWhatIf, onCreateReport }: { input: AnalysisInput; mode: Mode; onModeChange: (mode: Mode) => void; disabled: boolean; loading: boolean; result: AnalysisResponse | null; recentAnalyses: RecentAnalysis[]; onOpenRecent: (analysis: RecentAnalysis) => void; onRunWhatIf: (scenario: AnalysisInput) => Promise<AnalysisResponse | null>; onApplyWhatIf: (result: AnalysisResponse) => void; onCreateReport: () => void }) {
  const [historyOpen, setHistoryOpen] = useState(false);
  const [whatIfOpen, setWhatIfOpen] = useState(false);
  const closeWhatIf = useCallback(() => setWhatIfOpen(false), []);
  function openWhatIf() { setHistoryOpen(false); setWhatIfOpen(true); }
  return <aside className="workflow-card action-card">
    ...
    <div className="last-analysis"><Clock3 size={15}/><div><span>Aktueller Status</span><b>{result ? "Ergebnis verfügbar" : "Bereit zur Analyse"}</b></div></div>
